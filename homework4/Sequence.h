...
template <typename T> Sequence<T>::Sequence() : m_size{0} { createEmpty(); }
...
template <typename T> Sequence<T>::Sequence(const Sequence &other) : m_size{0} {
  createEmpty();
...
template <typename T> Sequence<T> &Sequence<T>::operator=(const Sequence &rhs) {
  if (this != &rhs) {
    Sequence temp(rhs);
    swap(temp);
  }
...
template <typename T> int Sequence<T>::insert(int pos, const T &value) {
  if (pos < 0 || pos > m_size) {
    return -1;
  }
...
template <typename T> int Sequence<T>::insert(const T &value) {
...
template <typename T> bool Sequence<T>::erase(int pos) {
  if (pos < 0 || pos >= m_size) {
    return false;
  }
...
template <typename T> int Sequence<T>::remove(const T &value) {
  int count = 0;
...
template <typename T> bool Sequence<T>::get(int pos, T &value) const {
  if (pos < 0 || pos >= m_size) {
    return false;
  }
...
template <typename T> bool Sequence<T>::set(int pos, const T &value) {
  if (pos < 0 || pos >= m_size) {
    return false;
  }
...
template <typename T> int Sequence<T>::find(const T &value) const {
...
template <typename T> void Sequence<T>::swap(Sequence &other) {
  // Swap head pointers
...
template <typename T> void Sequence<T>::createEmpty() {
  m_size = 0;

  // Create dummy node

  m_head = new Node; // NOLINT(cppcoreguidelines-owning-memory)
  m_head->m_next = m_head;
  m_head->m_prev = m_head;
}

template <typename T> void Sequence<T>::insertBefore(Node *p, const T &value) {
  // Create a new node

  Node *newp = new Node; // NOLINT(cppcoreguidelines-owning-memory)
