...
const char *COMMAND_FILE_NAME = "project4/commands.txt";
...
int main() {
  ...
  if (!thoroughf) {
    cout << "Cannot open " << COMMAND_FILE_NAME
         << ", so cannot do thorough correctness or performance tests!" << endl;
    return 1;
  }
...
struct EnterScopeCmd : public Command {
  EnterScopeCmd(string line, int lineno) : Command(line, lineno) {}
  virtual void execute(NameTable &nt) const { nt.enterScope(); }
...
struct ExitScopeCmd : public Command {
  ExitScopeCmd(string line, int lineno) : Command(line, lineno) {}
  virtual void execute(NameTable &nt) const { nt.exitScope(); }
  virtual bool executeAndCheck(NameTable &nt, SlowNameTable &snt) const {
...
struct DeclareCmd : public Command {
  ...
  virtual void execute(NameTable &nt) const { nt.declare(m_id, m_lineNum); }
  virtual bool executeAndCheck(NameTable &nt, SlowNameTable &snt) const {
...
struct FindCmd : public Command {
  ...
  virtual void execute(NameTable &nt) const { nt.find(m_id); }
  virtual bool executeAndCheck(NameTable &nt, SlowNameTable &snt) const {
...
void testPerformance(const vector<Command *> &commands) {
  ...
  cout << end << " milliseconds." << endl
       ...
       << "       Commands: " << (endCommands - endConstruction) << " msec."
       << endl
